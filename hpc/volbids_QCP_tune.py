...
MODEL_TYPES = ["bart", "tcn", "bayesian_ridge"]
...
OUTPUT_DIR = Path("results") / "volbids_QCP_tuning"
...
def _load_prediction_frame(model_type, zone, y_calibration, y_validation, rng):
    if model_type == "bart":
        return pd.read_parquet(f"predictions/bart/bart_predictions_CP_all_Normal_{zone}_volbids.parquet")

    common_index = y_calibration.index.union(y_validation.index)

    if model_type == "tcn":
        mask = tcn_cp.columns.str.startswith(f"{zone} | volbids")
    ...
    if model_type == "bayesian_ridge":
        mask = BR_cp.columns.str.startswith(f"{zone} | volbids")
...
def prepare_job_context(model_type, zone):
    feature, target = load_data(zone, "volbids")
...
def sample_conformalized_distribution(preds, alphas, cfg, context=JOB_CONTEXT, rng=None):
    ...
    volbid_samples = np.zeros((len(interpolators), N_SAMPLES))
    for i, intp in enumerate(interpolators):
        u = rng.uniform(size=N_SAMPLES)
        volbid_samples[i] = intp(u)

    return y_scaler.inverse_scale(volbid_samples)
...
def main():
    ...
    study_name = f"volbids_QCP_tuning_{safe_name(model_type)}_{safe_name(zone)}"

    print(
        "Running Optuna volbids tuning | "
